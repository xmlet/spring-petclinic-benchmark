...
export default class PetEditor extends React.Component<IPetEditorProps, IPetEditorState> {
  ...
  onSubmit(event) {
    ...
    const request: IPetRequest = {
      ...
      type: {
          id: Number(editablePet.typeId),
          name: selectedType ? selectedType.name : ''
        }
    };

    console.log('request being sent:', JSON.stringify(request));

    const submitUrl = editablePet.isNew ? '/api/owners/' + owner.id + '/pets' :  '/api/owners/' + owner.id + '/pets/' + editablePet.id;
    submitForm(editablePet.isNew ? 'POST' : 'PUT', submitUrl, request, (status, response) => {
